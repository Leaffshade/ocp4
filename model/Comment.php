...
class Comment extends Db
{
    ...
    public function addComment($description, $author, $articleId)
    {
        ...
        $query = $this->db->prepare(
            'INSERT INTO comment(description, date, author, article_id, notified) VALUES(?, NOW(), ?, ?, 0)'
        );
    ...
    {
        $query = $this->db->prepare(
            'UPDATE comment SET notified = 1 WHERE id = ?'
        );
        $query->execute([$id]);
    }
    ...
    public function notnotifyComment($id)
    {
        //Requetes SQL SELECT dans la table article pour récupérer tous les commentaires (page index)
        $query = $this->db->prepare(
            'UPDATE comment SET notified = 0 WHERE id = ?'
        );
        $query->execute([$id]);
    }
}
